import { Patient, Diagonosis } from './types';
...
//   const object = toNewPatient(obj) as NewPatient;
//   const patient = { obj,...object}
//   return object
// })

export const patients: Array<Patient> = patientsData.map((obj) => {
  const returnedObject = toNewPatient(obj);
  const id = obj.id;
  const patient = { id, ...returnedObject };

  return patient;
});

export const diagonoses: Array<Diagonosis> = [
  {
    code: 'M24.2',
